...
### 14.4.1 시그널 알림 중복 방지 (권장 패턴)

RSI 같은 조건 플러그인의 `below`/`above`는 **레벨 트리거**라, 조건이 유지되는 동안
스케줄 주기마다 시그널이 발생한다 (예: RSI가 30 아래에 3시간 머물면 5분 주기 폴링 시
알림 36건). 알림(텔레그램 등) 용도에는 **크로스 트리거**를 사용하라:
...
- `cross_below`: 직전 캔들 RSI ≥ 30 이고 현재 < 30 — **하향 돌파 순간 1회만** 통과 (RSI v3.1.0+)
- `cross_above`: 상향 돌파 순간 1회만 통과
- 레벨 트리거(`below`/`above`)는 "조건 유지 중 계속 실행"이 필요한 매매 로직에 사용

보조 수단으로 `ThrottleNode`의 `interval_sec`가 최대 86,400초(24시간)까지 지원되므로
정기 리포트성 알림은 `ThrottleNode(interval_sec=3600)` 등으로 묶을 수 있다.
단, Throttle은 쿨다운 동안 **새로운 이벤트도 함께 차단**하므로 시그널 알림의 1차
수단으로는 크로스 트리거를 권장.
