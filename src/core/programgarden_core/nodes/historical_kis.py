"""
ProgramGarden Core - KIS Historical Data Node

한국투자증권 일봉 조회:
- KisHistoricalDataNode: 국내주식 기간별 시세 (FHKST03010100) — RSI/Bollinger 등 지표 계산용
"""
...
KIS_CANDLE_FIELDS: List[Dict[str, str]] = [
...
KIS_TIME_SERIES_FIELDS: List[Dict[str, str]] = [
...
class KisHistoricalDataNode(BaseNode):
    """
    한국투자증권 일봉 REST 조회 노드

    KIS 국내주식 기간별 시세 TR(FHKST03010100)로 일봉 데이터를 조회합니다.
    ...
    KIS 응답은 최신 캔들이 배열 앞쪽에 옵니다. ConditionNode 플러그인은 oldest-first를
    기대하므로 executor에서 자동으로 역순 정렬합니다.
    """

    type: Literal["KisHistoricalDataNode"] = "KisHistoricalDataNode"
    category: NodeCategory = NodeCategory.MARKET
    description: str = "i18n:nodes.KisHistoricalDataNode.description"
    ...
    _usage: ClassVar[Dict[str, Any]] = {
        "when_to_use": [
            "RSI · Bollinger 등 지표 계산을 위한 일봉 OHLCV 데이터 수집 (KIS 계정)",
            "92번 예제: RSI 과매도 자동매수 봇의 캔들 소스",
        ],
        "when_not_to_use": [
            "실시간 현재가 → KisMarketDataNode 사용",
            "LS증권 계정의 일봉 조회 — KoreaStockHistoricalDataNode 계열 사용",
        ],
        "typical_scenarios": [
            "KisHistoricalDataNode → ConditionNode(RSI, period=14, threshold=30)",
            "KisHistoricalDataNode → ConditionNode(BollingerBands) → ThrottleNode → KisNewOrderNode",
        ],
    }
    ...
    _anti_patterns: ClassVar[List[Dict[str, str]]] = [
        ...
        },
        {
            ...
            "alternative": "현재가는 KisMarketDataNode를 사용하세요.",
        },
    ]
    _examples: ClassVar[List[Dict[str, Any]]] = [
        {
            ...
            "workflow_snippet": {
                "id": "kis-historical-rsi-bot",
                "name": "KIS RSI 매수 봇",
                "nodes": [
                    {"id": "start", "type": "StartNode"},
                    {"id": "broker", "type": "KisBrokerNode", "credential_id": "kis_cred", "paper_trading": True},
                    {"id": "candles", "type": "KisHistoricalDataNode", "symbol": "005930", "count": 30},
        ...
        },
        {
            ...
            "workflow_snippet": {
                "id": "kis-historical-chart",
                "name": "KIS 일봉 차트",
                "nodes": [
                    {"id": "start", "type": "StartNode"},
                    {"id": "broker", "type": "KisBrokerNode", "credential_id": "kis_cred", "paper_trading": True},
                    {"id": "candles", "type": "KisHistoricalDataNode", "symbol": "005930", "count": 60},
    ...
    _node_guide: ClassVar[Dict[str, Any]] = {
        ...
        "common_combinations": [
            "KisHistoricalDataNode → ConditionNode(RSI/BollingerBands) → ThrottleNode → KisNewOrderNode",
            "KisHistoricalDataNode → CandlestickChartNode (일봉 차트)",
            "ScheduleNode → KisHistoricalDataNode (장 마감 후 일봉 갱신)",
        ],
        "pitfalls": [
            "KIS 기간별시세 TR은 1회 최대 100건 — count>100은 잘림",
            "time_series는 oldest-first로 자동 정렬됨 — values는 KIS 원본 순서(최신 먼저)",
    ...
    _outputs: List[OutputPort] = [
        OutputPort(
            ...
            fields=KIS_TIME_SERIES_FIELDS,
        ),
        OutputPort(
            name="values",
            type="candle_data",
            description="i18n:ports.values",
            fields=KIS_CANDLE_FIELDS,
        ),
    ]
    ...
    def get_field_schema(cls) -> Dict[str, "FieldSchema"]:
        ...
        return {
            "symbol": FieldSchema(
                ...
                description="i18n:fields.KisHistoricalDataNode.symbol",
                required=True,
            ...
            "count": FieldSchema(
                ...
                description="i18n:fields.KisHistoricalDataNode.count",
                required=False,
                expression_mode=ExpressionMode.BOTH,
                category=FieldCategory.PARAMETERS,
                default=30,
