"""Pydantic models for LS Securities OpenAPI t1960 (ELW등락율상위(t1960)).
...
class T1960RequestHeader(BlockRequestHeader):
    pass


class T1960ResponseHeader(BlockResponseHeader):
    pass

class T1960InBlock(BaseModel):
    """t1960InBlock — input block. 기본입력"""
...
class T1960OutBlock1(BaseModel):
    """t1960OutBlock1 — output block (occurs — list of rows). 출력1"""
    ...
    bepoint: float = Field(
        default=0.0,
        title="손익분기점",
        description="bepoint — double(12.2). Consume as returned by LS.",
    )

class T1960Request(BaseModel):
    """Request envelope for t1960."""

    header: T1960RequestHeader = T1960RequestHeader(
    ...
    options: SetupOptions = SetupOptions(
        ...
        on_rate_limit="wait",
        rate_limit_key="t1960",
    )
    _raw_data: Optional[Response] = PrivateAttr(default=None)

class T1960Response(BaseModel):
    """Response envelope for t1960."""

    header: Optional[T1960ResponseHeader] = None
    block: Optional[T1960OutBlock] = None
    block1: List[T1960OutBlock1] = []
    rsp_cd: str = ""
    rsp_msg: str = ""
