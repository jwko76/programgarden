"""Pydantic models for LS Securities OpenAPI t1971 (ELW현재가호가조회(t1971)).
...
class T1971RequestHeader(BlockRequestHeader):
    pass


class T1971ResponseHeader(BlockResponseHeader):
    pass

class T1971InBlock(BaseModel):
    """t1971InBlock — input block. 기본입력"""
...
class T1971OutBlock(BaseModel):
    ...
    yevolume: float = Field(
        default=0.0,
        title="예상체결수량",
        description="yevolume — float(12). Consume as returned by LS.",
    )
    yesign: str = Field(
        default="",
        title="예상체결전일구분",
    ...
    invidx: str = Field(
        default="",
        title="ELW권리형태(1:표준2:디지털3:조기종료)",
        description="invidx — char(1). Consume as returned by LS.",
    )
    koba_stdprc: float = Field(
        default=0.0,
        title="KO베리어",
        description="koba_stdprc — float(12.2). Consume as returned by LS.",
    )
    koba_acc_rt: float = Field(
        default=0.0,
        title="KO접근도",
        description="koba_acc_rt — float(12.2). Consume as returned by LS.",
    )
    koba_yn: str = Field(
        default="",
        title="KO발생여부(Y/N)",
        description="koba_yn — char(1). Consume as returned by LS.",
    )

class T1971Request(BaseModel):
    """Request envelope for t1971."""

    header: T1971RequestHeader = T1971RequestHeader(
    ...
    options: SetupOptions = SetupOptions(
        ...
        on_rate_limit="wait",
        rate_limit_key="t1971",
    )
    _raw_data: Optional[Response] = PrivateAttr(default=None)

class T1971Response(BaseModel):
    """Response envelope for t1971."""

    header: Optional[T1971ResponseHeader] = None
