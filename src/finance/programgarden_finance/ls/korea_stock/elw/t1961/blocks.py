"""Pydantic models for LS Securities OpenAPI t1961 (ELW거래량상위(t1961)).
...
class T1961RequestHeader(BlockRequestHeader):
    pass


class T1961ResponseHeader(BlockResponseHeader):
    pass

class T1961InBlock(BaseModel):
    """t1961InBlock — input block. 기본입력"""
...
class T1961OutBlock1(BaseModel):
    """t1961OutBlock1 — output block (occurs — list of rows). 출력1"""
    ...
    jnilvolume: float = Field(
        default=0.0,
        title="전일거래량",
        description="jnilvolume — double(12.0). Consume as returned by LS.",
    )
    elwexec: float = Field(
        default=0.0,
        title="행사가",
...
class T1961Request(BaseModel):
    """Request envelope for t1961."""

    header: T1961RequestHeader = T1961RequestHeader(
    ...
    options: SetupOptions = SetupOptions(
        ...
        on_rate_limit="wait",
        rate_limit_key="t1961",
    )
    _raw_data: Optional[Response] = PrivateAttr(default=None)

class T1961Response(BaseModel):
    """Response envelope for t1961."""

    header: Optional[T1961ResponseHeader] = None
    block: Optional[T1961OutBlock] = None
    block1: List[T1961OutBlock1] = []
    rsp_cd: str = ""
    rsp_msg: str = ""
