...
logger = logging.getLogger("programgarden.ls.t1514")
from .blocks import (
    T1514RequestHeader,
    T1514ResponseHeader,
    T1514InBlock,
...
class TrT1514(OccursReqAbstract, TRRequestAbstract):
    """LS증권 OpenAPI t1514 — 업종기간별추이(t1514)"""

    def __init__(self, request_data: T1514Request):
        super().__init__(
        ...
        self._generic: GenericTR[T1514Response] = GenericTR[T1514Response](
    ...
    def _build_response(
        ...
        block_data = resp_json.get("t1514OutBlock")
        ...
        if exc is None and not is_error:
            block = T1514OutBlock.model_validate(block_data) if block_data is not None else None
            block1 = [T1514OutBlock1.model_validate(r) for r in block1_data]
        ...
        if exc is not None:
            ...
            logger.error(f"t1514 request failed: {error_msg}")
