"""키움증권(Kiwoom Securities) TR 요청을 위한 추상 클래스입니다.

KIS 패키지의 ``kis/tr_base.py`` 슬라이딩 윈도우 rate-limit 구조를
브로커 독립적인 구현이므로 그대로 재사용합니다 (원본은 빗썸 패키지의
``bithumb/tr_base.py`` 구조를 따름). 키움 API의 정확한 rate-limit 정책은
공식 문서로 확인되지 않아 ``SetupOptions`` 기본값은 보수적인 추정치입니다.
TODO(실계좌 검증): 정확한 rate-limit 정책 확인.
"""
...
logger = logging.getLogger("programgarden.kiwoom.tr_base")


TRresponse = TypeVar("TRresponse")


class KiwoomTRRequestAbstract(ABC):
    """키움 TR 요청 추상 클래스 (슬라이딩 윈도우 rate-limit 포함)."""
    ...
    def __init__(
        ...
        if rate_limit_key:
            shared = KiwoomTRRequestAbstract._shared_rate_data.get(rate_limit_key)
            if shared is None:
                lock = threading.RLock()
                shared = {"lock": lock, "cond": threading.Condition(lock), "timestamps": []}
                KiwoomTRRequestAbstract._shared_rate_data[rate_limit_key] = shared
        ...
        try:
            if self.on_rate_limit == "stop":
                if self.is_rate_limited():
                    raise ValueError("Kiwoom rate limit exceeded")
                self.record_request()
            else:
            ...
            logger.error(f"Kiwoom 동기 요청 실패: {e}")
            raise
    ...
    async def execute_async_with_session(
        ...
        try:
            ...
            logger.error(f"Kiwoom 비동기 요청 실패: {e}")
            raise
