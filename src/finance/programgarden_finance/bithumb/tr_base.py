"""빗썸(Bithumb) TR 요청을 위한 추상 클래스입니다.

LS증권 패키지의 ``ls/tr_base.py``에 있는 슬라이딩 윈도우 rate-limit 로직을
재사용하되, 계정 누적 rate-limit 게이트는 제외하고 HTTP 메서드
(GET/POST/DELETE)와 쿼리 파라미터/JSON body를 일반화했습니다.
"""
...
logger = logging.getLogger("programgarden.bithumb.tr_base")


TRresponse = TypeVar("TRresponse")


class BithumbTRRequestAbstract(ABC):
    """빗썸 TR 요청 추상 클래스 (슬라이딩 윈도우 rate-limit 포함)."""
    ...
    def __init__(
        ...
        if rate_limit_key:
            shared = BithumbTRRequestAbstract._shared_rate_data.get(rate_limit_key)
            if shared is None:
                lock = threading.RLock()
                shared = {"lock": lock, "cond": threading.Condition(lock), "timestamps": []}
                BithumbTRRequestAbstract._shared_rate_data[rate_limit_key] = shared
        ...
        try:
            if self.on_rate_limit == "stop":
                if self.is_rate_limited():
                    raise ValueError("Bithumb rate limit exceeded")
                self.record_request()
            else:
