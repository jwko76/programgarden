"""빗썸 출금 리스트 조회 (GET /v1/withdraws) 요청/응답 모델입니다."""
...
class WithdrawsRequest(BaseModel):
    """출금 리스트 조회 요청입니다."""

    params: WithdrawsInBlock = Field(default_factory=WithdrawsInBlock, description="요청 파라미터")
...
class WithdrawOutBlock(BaseModel):
    """출금 리스트 조회 응답 항목입니다."""

    type: str = Field(..., title="입출금 종류", examples=["withdraw"])
    uuid: str = Field(..., title="출금에 대한 고유 아이디", examples=["94332e99-3a87-4a35-ad98-28b0c969f830"])
    ...
    created_at: str = Field(..., title="출금 생성 시간", examples=["2026-06-10T12:30:00+09:00"])
    done_at: Optional[str] = Field(default=None, title="출금 완료 시간", examples=["2026-06-10T12:40:00+09:00"])
    amount: str = Field(..., title="출금 금액/수량", examples=["1.0"])
    fee: str = Field(..., title="출금 수수료", examples=["0.0005"])
    transaction_type: str = Field(..., title="출금 유형", description="default(일반출금)/internal(바로출금)", examples=["default"])


class WithdrawsResponse(BithumbResponseBase):
    """출금 리스트 조회 응답입니다."""

    blocks: Optional[List[WithdrawOutBlock]] = Field(default=None, description="출금 내역 목록")
