...
- [ ] **키움증권 라이브 검증 — 잔여분** (2026-07-18 모의서버 1차 검증 완료, 아래 "완료됨" 참조)
  ...
  - [ ] **실전 서버 + 빗썸 개인 API** — 로컬 PC IP가 화이트리스트에 없어 거부(키움 8050 / 빗썸 not allowed client IP).
    라이브 검증은 OCI 개발서버(`infra/oci-dev/`, 고정 IP)에서 실행하기로 결정 (2026-07-19 프로비저닝 완료).
    **사용자 액션 필요**: 개발서버 고정 IP(`terraform -chdir=infra/oci-dev output -raw instance_public_ip`)를
    키움 지정단말기·빗썸 API 허용 IP에 **추가** 등록 후 재시도
